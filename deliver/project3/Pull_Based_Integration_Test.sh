...
Server_Name=0
Gnutella_Name=0
export PUSH_BASED_CONSISTENCY=FALSE
export TTR=450
export PROFILE=FALSE
chmod 775 "$( cd "$( dirname "$0" )" && pwd )"/Instrument/RMIClient/pull_based_integration_test.py
...
find ../ -type f -name "*.txt" -exec touch {} +
# Integration test
echo "PULL BASED CONSISTENCY Integ testing running..."
cd "Instrument/RMIClient" && python3 "$( cd "$( dirname "$0" )" && pwd )"/Instrument/RMIClient/pull_based_integration_test.py